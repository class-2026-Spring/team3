...
export default function ChargerList({
  ...
  if (level === 'city') {
    ...
    return (
      <div className="flex flex-col bg-white dark:bg-[#1a1d27] h-full">
        ...
        <div className="p-4 overflow-y-auto flex-1 bg-gray-50 dark:bg-[#13151f] flex flex-col gap-3">
          ...
          {[
            ...
            return (
              <div key={name} onClick={() => selectCity(name as '제주시' | '서귀포시')}
                className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1a1d27] cursor-pointer hover:border-blue-300 hover:shadow-md transition-all group">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ background: color }} />
                    <span className="text-[15px] font-extrabold text-gray-900 dark:text-white">{name}</span>
                  </div>
                  <span className="text-[11px] text-gray-400 group-hover:text-blue-500 transition-colors">선택 →</span>
                </div>
                <div className="flex gap-3">
                  <div className="flex-1 bg-gray-50 dark:bg-gray-800 rounded-lg p-2.5 text-center">
                    <p className="text-[20px] font-black text-gray-800 dark:text-white">{list.length}</p>
                    <p className="text-[10px] font-semibold text-gray-400 mt-0.5">전체 충전소</p>
                  </div>
                  <div className="flex-1 bg-orange-50 dark:bg-orange-900/20 rounded-lg p-2.5 text-center">
  ...
  if (level === 'district') {
    ...
    return (
      <div className="flex flex-col bg-white dark:bg-[#1a1d27] h-full">
        ...
        <div className="p-4 overflow-y-auto flex-1 bg-gray-50 dark:bg-[#13151f]">
            ...
            {sortedDistricts.map(([districtName, list]) => {
              ...
              return (
                <div key={districtName} onClick={() => selectDistrict(districtName)}
                  className="px-3.5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1a1d27] flex items-center justify-between cursor-pointer hover:border-teal-300 hover:shadow-sm transition-all group">
                  <div>
                    <div className="flex items-center gap-1.5">
                      <p className="text-[14px] font-bold text-gray-900 dark:text-white group-hover:text-teal-600 transition-colors">{districtName}</p>
                      {favCount > 0 && (
                    ...
                    <div className="flex gap-1.5 mt-1">
                      {fast > 0 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-orange-50 dark:bg-orange-900/20 text-orange-500 border border-orange-100 dark:border-orange-800">급속 {fast}</span>}
                      {slow > 0 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-500 border border-blue-100 dark:border-blue-800">완속 {slow}</span>}
                    </div>
                  </div>
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-[#1a1d27] h-full">
      ...
      <div className="p-4 overflow-y-auto flex-1 bg-gray-50 dark:bg-[#13151f]">
        <p className="text-[13px] font-bold text-gray-800 dark:text-white mb-3 flex items-center gap-1.5">
            ...
            {sortedChargers.slice(0, 100).map(charger => {
              ...
              return (
                <div key={charger.id} onClick={() => onSelectCharger(charger)}
                  className={`p-3.5 rounded-xl border bg-white dark:bg-[#1a1d27] flex justify-between items-center cursor-pointer hover:shadow-md transition-all group ${fav ? 'border-yellow-200 dark:border-yellow-800 bg-yellow-50/20 dark:bg-yellow-900/10' : 'border-gray-200/60 dark:border-gray-700 hover:border-teal-300'}`}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-[14px] font-bold text-gray-900 dark:text-white group-hover:text-teal-600 transition-colors truncate">{charger.name}</p>
                    ...
                    <div className="flex gap-2 mt-1.5">
                      {stats.fastTotal > 0 && <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-orange-50 dark:bg-orange-900/20 text-orange-600 border border-orange-100 dark:border-orange-800">급속 {stats.fastAvail}/{stats.fastTotal}</span>}
                      {stats.slowTotal > 0 && <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-900/20 text-blue-600 border border-blue-100 dark:border-blue-800">완속 {stats.slowAvail}/{stats.slowTotal}</span>}
                    </div>
                  </div>
                  <div className="ml-2 shrink-0 flex flex-col items-end gap-1.5">
                    <button onClick={e => { e.stopPropagation(); onToggleFavorite(charger); }} className="p-1 rounded-full hover:bg-yellow-50 dark:hover:bg-yellow-900/20 transition-colors">
      ...
      <div className="flex gap-2 bg-gray-50 dark:bg-gray-800 p-1 rounded-lg">
        {(['전체', '급속', '완속'] as FilterType[]).map(f => (
          <button key={f} onClick={() => setChargeFilter(f)}
            className={`text-xs px-3 py-1.5 rounded-md font-semibold transition-all ${chargeFilter === f ? 'bg-white dark:bg-gray-700 text-teal-600 shadow-sm border border-gray-200/60 dark:border-gray-600' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
            {f}
          </button>
        ))}
      </div>
