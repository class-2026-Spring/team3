...
export default function SettingsPanel({ open, onClose }: Props) {
  ...
  const Toggle = ({
    ...
    >
      <span
        className={`absolute top-[3px] w-4 h-4 bg-white dark:bg-gray-100 rounded-full shadow transition-transform duration-200 ${
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
            value === opt.value
              ...
              : 'border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-teal-300 dark:hover:border-teal-700 hover:text-teal-600 dark:hover:text-teal-400'
          }`}
        >
          {opt.label}
        </button>
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="fixed right-0 top-0 h-full w-[300px] bg-white dark:bg-gray-900 shadow-2xl z-50 flex flex-col transition-colors"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[14px] font-bold text-gray-800 dark:text-gray-100">{t('settings.title')}</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
          ...
          {/* 언어 */}
          <section>
            <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">{t('settings.language')}</p>
            <ChipGroup<Language>
              options={[
                { value: 'ko', label: '한국어' },
          ...
          {/* 테마 */}
          <section>
            <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">{t('settings.theme')}</p>
            <ChipGroup<Theme>
              options={[
              ...
              <div>
                ...
                <ChipGroup<MapFilter>
                  options={[
                    ...
                    { value: '급속', label: t('settings.mapFilter_fast') },
                    { value: '완속', label: t('settings.mapFilter_slow') },
                  ]}
          ...
          {/* 알림 설정 */}
          <section>
            <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">{t('settings.notifications')}</p>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-gray-600 dark:text-gray-300 font-semibold">{t('settings.notif_enabled')}</p>
                </div>
                <Toggle
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-xs font-semibold ${settings.notifications.enabled ? 'text-gray-600 dark:text-gray-300' : 'text-gray-300 dark:text-gray-600'}`}>
                    {t('settings.notif_status')}
                  </p>
                </div>
                <Toggle
