...
export default function Sidebar({ user, unreadCount, onSignIn, onSignOut, onSettings, onNotifications, showMobile, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      {/* 데스크탑 사이드바 */}
      <aside className="w-[260px] bg-white dark:bg-gray-900 hidden lg:flex flex-col m-4 rounded-[20px] shadow-sm border border-gray-100/50 dark:border-gray-800 overflow-y-auto scrollbar-hide z-50 transition-colors">
        <SidebarContent
          pathname={pathname}
          session={session}
          profile={profile}
      ...
      {/* 모바일 사이드바 */}
      {showMobile && (
        <aside className="w-[260px] bg-white dark:bg-gray-900 flex flex-col lg:hidden fixed top-4 bottom-4 left-4 rounded-[20px] shadow-xl border border-gray-100/50 dark:border-gray-800 overflow-y-auto scrollbar-hide z-50 transition-colors">
          <SidebarContent
            pathname={pathname}
            session={session}
            profile={profile}
  ...
  return (
    <>
      ...
      <div className="p-6 pb-4 text-center flex items-center justify-center gap-3">
        ...
        <span className="font-extrabold text-gray-800 dark:text-gray-100 text-[13px] tracking-widest uppercase">JEJU EV MAP</span>
      </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto scrollbar-hide">
        ...
        >
          <p className="text-[11px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-wider group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors">{t('sidebar.main')}</p>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-300 dark:text-gray-600 transition-transform duration-200 ${isMainOpen ? 'rotate-180' : ''}`}><polyline points="6 9 12 15 18 9"></polyline></svg>
        </div>
        ...
        <div className={`space-y-1 mb-2 overflow-hidden transition-all duration-300 ${isMainOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
          <Link href="/" onClick={onCloseMobile} className={getTabClass("/")}>
            ...
            <span className="text-[13px]">{t('sidebar.map')}</span>
          </Link>
        ...
        >
          <p className="text-[11px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-wider group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors">{t('sidebar.profile')}</p>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-300 dark:text-gray-600 transition-transform duration-200 ${isProfileOpen ? 'rotate-180' : ''}`}><polyline points="6 9 12 15 18 9"></polyline></svg>
        </div>
        ...
        <div className={`space-y-1 overflow-hidden transition-all duration-300 ${isProfileOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
          <Link href="/favorite" onClick={onCloseMobile} className={getTabClass("/favorite")}>
            ...
            <span className="text-[13px]">{t('sidebar.favorites')}</span>
          </Link>
          ...
          <Link href="/mycomments" onClick={onCloseMobile} className={getTabClass("/mycomments")}>
            ...
            <span className="text-[13px]">{t('sidebar.myComments')}</span>
          </Link>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={onSettings} className="w-8 h-8 rounded-[10px] bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors border border-gray-100 dark:border-gray-700" title={t('sidebar.settings')}>
          ...
          <button onClick={onNotifications} className="relative w-8 h-8 rounded-[10px] bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors border border-gray-100 dark:border-gray-700" title={t('sidebar.notifications')}>
        ...
        {session ? (
          ...
          <button onClick={onSignIn} className="flex items-center gap-1.5 text-teal-500 hover:text-teal-400 font-bold text-xs transition-colors">
      ...
      {session && profile && (
        <div className="p-4 mt-auto border-t border-gray-100 dark:border-gray-800">
          <div
            ...
            className="flex items-center gap-3 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-colors group"
          >
            <div className="relative">
              <img src={getAvatarUrl(profile.avatar_url)} alt="Avatar" className="w-10 h-10 rounded-full object-cover border-[2.5px] border-teal-100/60 dark:border-teal-900 bg-gray-100 dark:bg-gray-800" />
              <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-teal-500 border-2 border-white dark:border-gray-900 rounded-full"></div>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-extrabold text-gray-800 dark:text-gray-100 truncate">{profile.nickname}</p>
              <p className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 truncate">{session.user?.email || 'EV Driver'}</p>
            </div>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-300 dark:text-gray-600 group-hover:text-gray-600 dark:group-hover:text-gray-400 transition-colors shrink-0"><polyline points="6 9 12 15 18 9"></polyline></svg>
          </div>
        </div>
      )}
    </>
